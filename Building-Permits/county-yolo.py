...
# %% Read In Raw Table
# yolo county
test = 'county-yolo-final.csv'
raw = pd.read_csv(root + test)
print(raw.columns.to_list())

# %% List Desired Fields
fields= ['PARCEL NUMBER', 'PERMIT NUMBER', 'ADDRESS', 'TYPE', 'SUBTYPE', 
'ISSUED_BY', 'APPLIED_BY', 'DESCRIPTION']
...
out_df['county_name'] = 'Yolo County'
...
out_df['permit_class'] = df.loc[:,'TYPE']

#%% populate permit type
out_df['permit_type'] = df.loc[:,'SUBTYPE']
