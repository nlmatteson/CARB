...
# %% Read In Raw Table
# oceanside data
test = 'city-oceanside-final.csv'
raw = pd.read_csv(root + test)
print(raw.columns.to_list())
# %% List Desired Fields
fields = ['Permit Number', 'Application Date', 'Approved Date',
'Issued Date', 'Finaled Date', 'Expiration Date',
'Permit Type', 'Permit Subtype', 'Status', 'APN', 'Address']
...
out_df['permit_type'] = df.loc[:,'Permit Type']

# %% Populate Applied Date Filed

out_df ['applied_date'] = df.loc[:,'Application Date']
...
out_df['parcel_number'] = df.loc[:,'APN']
