...
# %% Read In Raw Table
# san rafael
test = 'city-san_rafael-final.csv'
raw = pd.read_csv(root + test)
print(raw.columns.to_list())
# %% List Desired Fields
fields= ['Address', 'Permit Number', 'Parcel Number (no dashes)',
'Issue Date', 'Status', 'Description', 'RECORDID']
...
out_df.loc[:,'parcel_number'] = df.loc[:,'Parcel Number (no dashes)']
