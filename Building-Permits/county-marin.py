...
# %% Read In Raw Table
# marin county
test = 'county-marin-final.csv'
raw = pd.read_csv(root + test)
print(raw.columns.to_list())

# %% List Desired Fields
fields= ['Permit Number', 'Issued Date', 'Received Date',
'Address', 'Parcel Number', 'City Town', 'Construction Value',
'Construction',
'Description', 'Type Permit', 'Permit Category']
...
out_df['county_name'] = 'Marin County'
...
out_df['permit_class'] = df.loc[:,'Permit Category']

#%% populate permit type
out_df['permit_type'] = df.loc[:,'Construction']
# %% populate description
out_df['project_description']= df.loc[:,'Description']

#%% Populate Estimated Cost Field
out_df['estimated_cost'] = df['Construction Value']

# %%  Populate  Date Fieled
out_df ['applied_date'] = df.loc[:,'Received Date']
...
out_df['parcel_number']= df.loc[:,'Parcel Number']
